...
class ActivityRuntimeService {
  ...
  init() {
    clockRuntimeService.subscribe(now => {
      this.onTick(now)
    })
  }
    ...
    for (const [characterId, bucket] of this.activities.entries()) {
      ...
      for (const activity of bucket.values()) {
        ...
        eventBus.emit({
          ...
          parentEventId: activity.eventId,
  ...
  getProgress(characterId: string, activityId: string) {
    ...
    const elapsed = clockRuntimeService.getNow() - activity.startedAt
  ...
  start(entry: ActivityEntry) {
    ...
    bucket.set(entry.id, {
      ...
      status: 'active',
      startedAt: entry.startedAt ?? clockRuntimeService.getNow(),
    })
  ...
  complete(characterId: string, activityId: string) {
    ...
    activity.status = 'completed'
    activity.completedAt = clockRuntimeService.getNow()
